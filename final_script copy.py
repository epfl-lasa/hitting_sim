...
def plot_optimization(X_opt, environment, x_limits, y_limits, table_direction, colormap,\
    ...
    if colormap:
        ...
        colorbar.set_label('Probability to reach Xf given Xi and Xm')
    # Plot initial position of box


    X_opt[0] = (Xi[0]+Xf[0])/2
    X_opt[1] = (Xi[1]+Xf[1])/2
    X_opt= guess(Xi,Xf,table_direction)
    ...
    ax.scatter(X_opt[0],X_opt[1], s=100, marker='+',color='b', label ='Xm0')
    ax.plot([Xi[0],X_opt[0]],[Xi[1],X_opt[1]],color='k', linestyle='dashed')
    ax.plot([X_opt[0],Xf[0]],[X_opt[1],Xf[1]],color='k', linestyle='dashed')

    ax.plot([Xi[0],Xi[0]+0.5],[Xi[1],Xi[1]],color='k', linestyle='dashed')

    ax.plot([X_opt[0],X_opt[0]+0.5],[X_opt[1],X_opt[1]],color='k', linestyle='dashed')
    ...
    # Add the angle text
    angle_text = r'$\theta_{10}$'
    #ax.text(( X_opt[0]) / 1, (Xi[1])-0.1, angle_text, ha='right', va='bottom', color='purple')

    # Add the arc
    start_angle = 0  # Starting angle in degrees
    end_angle = angle  # Ending angle in degrees
    radius = 0.5  # Radius of the arc

    arc = patches.Arc((Xi[0], Xi[1]), radius*1.5 , radius*1.5 , 0, start_angle, end_angle, color='purple', linewidth=3, label=angle_text)
    ax.add_patch(arc)
    ...
    arc = patches.Arc((X_opt[0], X_opt[1]), radius*1.5 , radius*1.5 , 0, start_angle, end_angle, color='gray', linewidth=3, label=angle_text)
    ax.add_patch(arc)
    plt.xlabel('X-axis')
    plt.ylabel('Y-axis')
